...
| 7 | Alerta y asignación de tareas de limpieza | On each check-out registration, Make notifies the housekeeping team via WhatsApp with room number and priority. When they finish, they mark the task as done. | Make.com · WhatsApp · Airtable / Notion | Planned | ⚠️ Not applicable — GHP does a mass checkout each morning (443 rooms); housekeeping teams monitor hallways directly. Per-room alerts don't fit this workflow |
| 8 | Análisis semanal de reseñas con IA | Every week, Make collects reviews from Booking and TripAdvisor, runs them through Gemini to detect patterns (recurring complaints, strengths), and sends a one-page report to the management team. | Make.com · Gemini API · Sheets | Planned | ⚠️ Manual — staff currently copy reviews into ChatGPT to generate the weekly report |
...
| 10 | Emails VIP ultra-personalizados | Gemini analyzes a VIP guest's stay history and Make generates a fully personalized email before their next arrival, referencing their real preferences. | Gemini API · Make.com · CRM / Sheets | Planned | ⚠️ Limited data — only room type, regime, and dates tracked per guest; bar/restaurant consumption is aggregated. Mailchimp segments by age and occupation type available as a proxy |
